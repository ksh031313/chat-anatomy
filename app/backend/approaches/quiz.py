...
async def summarize_latest_chat_history(auth_claims: dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        prompt = (
            ...
            "Markdown 형식으로 구조화하여 1000자 이내로 요약해 주세요. 단, 출력에 삼중 백틱(예: ```markdown 또는 ```)과 같은 코드 블록 표기는 포함하지 마시고, 대신 헤더(#), 굵은 글씨(**), 목록(- 또는 1.), 링크 등 일반적인 Markdown 문법만 사용해 주세요.\n"
            "요약 처음에 한 줄로 '이 요약은 질문과 답변 내용을 요약한 것입니다.'라는 안내 문구를 포함해 주세요.\n"
            "대화 내용:\n"
            f"{history_text}\n"
            "요약:"
        )
